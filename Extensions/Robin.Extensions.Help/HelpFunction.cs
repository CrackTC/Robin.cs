...
{
    private static string GetTriggerDescription(BotFunction function, BotFunctionInfoAttribute info)
    {
        var infoText = string.Join(
            " 或 ",
            info.EventTypes.Select(type =>
                type.GetCustomAttribute<EventDescriptionAttribute>()!.Description
            )
        );

        var tunnelText = string.Concat(
            function.TriggerDescriptions.Select(tunnelDesc => $"• {tunnelDesc}\n")
        );

        if (string.IsNullOrEmpty(infoText) && string.IsNullOrEmpty(tunnelText))
            return string.Empty;
        ...
        if (!string.IsNullOrEmpty(infoText))
        {
            ...
            if (!string.IsNullOrEmpty(tunnelText))
                builder.Append("，或");
        }
    ...
    {
        var helps = _context
            .BotContext.Functions.Select(f =>
                (Func: f, Info: f.GetType().GetCustomAttribute<BotFunctionInfoAttribute>()!)
            )
            .ToDictionary(
                t => t.Info.Name,
                t =>
        ...
        builder
            ...
            .Do(async tuple =>
            {
                ...
                {
                    await e.NewMessageRequest([new TextData($"未找到功能：{name.Value}")])
                        .SendAsync(_context, t);
                    return;
                }
                ...
                if (
                    e switch
                    {
                        IGroupEvent { GroupId: var id } =>
                            !help.Func.Context.GroupFilter.IsIdEnabled(id),
                        IPrivateEvent { UserId: var id } =>
                            !help.Func.Context.PrivateFilter.IsIdEnabled(id),
                        _ => false,
                    }
                )
                {
                    await e.NewMessageRequest([
                            new TextData($"功能 {name.Value} 在当前上下文中被禁用"),
                        ])
                        .SendAsync(_context, t);
                    return;
                }
