...
public partial class GeminiFunction(FunctionContext<GeminiOption> context)
    : BotFunction<GeminiOption>(context),
        IFluentFunction
{
    ...
    public async Task OnCreatingAsync(FunctionBuilder builder, CancellationToken token)
    {
        try
        {
            ...
            _systemRegex = new Regex(
                _context.Configuration.SystemRegexString,
                RegexOptions.Compiled
            );
            _clearRegex = new Regex(_context.Configuration.ClearRegexString, RegexOptions.Compiled);
            _rollbackRegex = new Regex(
                _context.Configuration.RollbackRegexString,
                RegexOptions.Compiled
            );
        }
        ...
        builder
            ...
            .Do(async tuple =>
            {
                ...
                if (
                    await request.GenerateContentAsync(
                ...
                if (
                    response
                    is not GeminiGenerateDataResponse { Candidates: { Count: > 0 } candidates }
                )
                {
                    LogGenerateContentFailed(_context.Logger, e.UserId);
                ...
                {
                    if (!await SendReplyAsync(e.UserId, reply, t))
                        return;
    ...
    private async Task<bool> SendReplyAsync(long userId, string reply, CancellationToken token) =>
        await new SendPrivateMessage(userId, [new TextData(reply)]).SendAsync(_context, token)
            is not null;
    ...
    private Task<int> RemoveLastAsync(long userId, CancellationToken token) =>
        _semaphore.ConsumeAsync(
            ...
            {
                var last = _db
                    .Messages.Where(msg => msg.User.UserId == userId)
    ...
    [LoggerMessage(
        ...
        Message = "Failed to generate content for user {UserId}"
    )]
