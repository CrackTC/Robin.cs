...
{
    static int[] a = Str2Bits("1");
    static int[] b = Str2Bits("2");
    static int[] c = Str2Bits("3");
    ...
    {
        var paddedStr = str.PadRight(4, '\0').Substring(0, 4);
    ...
    private static int[] Enc(int[] data, int[] key)
    {
        ...
        return Final(r.Concat(l).ToArray());
    }

    private static int[] Init(int[] data) => new[]
    {
    ...
    private static int[] Expand(int[] data) => new[]
    {
        31, 0, 1, 2, 3, 4, 3, 4,
        5, 6, 7, 8, 7, 8, 9, 10,
    ...
    private static int[] P(int[] bytes) => new[]
    {
        15, 6, 19, 20, 28, 11, 27, 16,
        0, 14, 22, 25, 4, 17, 30, 9,
        1, 7, 23, 13, 31, 26, 2, 8,
        18, 12, 29, 5, 21, 10, 3, 24,
    }.Select(index => bytes[index]).ToArray();

    private static int[] Final(int[] endByte) => new[]
    {
    ...
    }.Select(index => endByte[index]).ToArray();

    private static int[][] GetKeys(int[] src)
    {
        var keys = new int[16][];
        var loop = new[] { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
        ...
        var key = new[]
        {
            ...
            61, 53, 45, 37, 29, 21, 13, 5,
            62, 54, 46, 38, 30, 22, 14, 6,
        }.Select(index => src[index]).ToArray();
        ...
        for (var i = 0; i < 16; i++)
        {
            ...
            keys[i] = new[]
            {
                ...
                43, 48, 38, 55, 33, 52,
                45, 41, 49, 35, 28, 31,
            }.Select(index => key[index]).ToArray();
        }
