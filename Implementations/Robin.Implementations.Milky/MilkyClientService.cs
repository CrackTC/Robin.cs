...
internal partial class MilkyClientService(IServiceProvider service, MilkyClientOption options)
    : BackgroundService,
        IBotEventInvoker,
        IOperationProvider
{
    private readonly ILogger<MilkyClientService> _logger = service.GetRequiredService<
        ILogger<MilkyClientService>
    >();

    private MilkyClient client = null!;

    private readonly MilkyJsonSerializerContext _milkyJsonSerializerContext = new(
    ...
    private static UserSex ConvertSex(Sex sex) =>
        sex switch
        {
    ...
    private static GroupMemberRole ConvertRole(Role role) =>
        role switch
        {
        ...
        where TResp : Response
    {
        switch (request)
        {
            ...
            {
                if (
                    deleteMsg.MessageId.Split('|')
                    is not ([var messageScene, var peerIdStr, var messageSeqStr])
                )
                ...
                {
                    case "friend":
                        await client.Message.RecallPrivateMessageAsync(
                            new(peerId, messageSeq),
                            token
                        );
                        return (Response.Shared as TResp)!;
                    case "group":
                        await client.Message.RecallGroupMessageAsync(
                            new(peerId, messageSeq),
                            token
                        );
                        return (Response.Shared as TResp)!;
                    default:
                        throw new NotSupportedException(
                            $"Unsupported message scene: {messageScene}"
                        );
                }
            }
            case GetFriendList getFriendList:
            {
                ...
                var resp = new GetFriendListResponse([
                    .. result.Friends.Select(friend => new FriendInfo(
                        friend.UserId,
                        friend.Nickname,
                        friend.Remark
                    )),
            ...
            {
                var result = await client.System.GetGroupInfoAsync(
                    new(getGroupInfo.GroupId, getGroupInfo.NoCache),
                    token
                );
                var resp = new GetGroupInfoResponse(
                    new GroupInfo(
                        ...
                        result.Group.MemberCount,
                        result.Group.MaxMemberCount
                    )
                );
                return (resp as TResp)!;
            }
            case GetGroupList getGroupList:
            {
                ...
                var resp = new GetGroupListResponse([
                    .. result.Groups.Select(group => new GroupInfo(
                        group.GroupId,
                        group.GroupName,
                        group.MemberCount,
            ...
            {
                var result = await client.System.GetGroupMemberInfoAsync(
                    new(
                        getGroupMemberInfo.GroupId,
                        getGroupMemberInfo.UserId,
                        getGroupMemberInfo.NoCache
                    ),
                    token
                );
            ...
            {
                var result = await client.System.GetGroupMemberListAsync(
                    new(getGroupMemberList.GroupId, getGroupMemberList.NoCache),
                    token
                );
            ...
            case SendGroupForwardMessage sendGroupForwardMsg:
            {
                ...
                return (
                    new SendGroupForwardMessageResponse(
                        new ForwardResult(
                            GenerateMessageId(
                                MessageScene.Group,
                                sendGroupForwardMsg.GroupId,
            ...
            {
                await client.Group.SendGroupNudgeAsync(
                    new SendGroupNudgeInput(sendGroupPoke.GroupId, sendGroupPoke.UserId),
                    token
                );
                return (Response.Shared as TResp)!;
            }
            ...
            case SetFriendAddRequest setFriendAddReq:
            {
                ...
                {
                    await client.Friend.AcceptFriendRequestAsync(
                        new(setFriendAddReq.Flag, false),
                        token
                    );
                }
                else
                {
                    await client.Friend.RejectFriendRequestAsync(
                        new(setFriendAddReq.Flag, false, setFriendAddReq.Remark),
                        token
                    );
                }
                return (Response.Shared as TResp)!;
            ...
            {
                await client.Group.SendGroupMessageReactionAsync(
                    new(
            ...
            {
                await client.File.UploadGroupFileAsync(
                    new(
    ...
    private static string GenerateMessageId(IncomingMessage msg) =>
        GenerateMessageId(GetMessageScene(msg), msg.PeerId, msg.MessageSeq);

    private MessageChain ConvertSegment(IncomingMessage msg) =>
        [
            .. msg.Segments.SelectMany(seg =>
                seg switch
                {
                    ...
                    RecordIncomingSegment record =>
                    [
                        new RecordData(File: record.Data.TempUrl, Url: record.Data.TempUrl),
                    ],
                    VideoIncomingSegment video =>
                    [
                        new VideoData(File: video.Data.TempUrl, Url: video.Data.TempUrl),
                    ],
    ...
    private static OutgoingSegment[] ConvertFromMessageChain(MessageChain chain) =>
        [
            .. chain.Select<SegmentData, OutgoingSegment>(seg =>
                seg switch
                {
                    ...
                    ReplyData reply => new ReplyOutgoingSegment(
                        new(Convert.ToInt64(reply.Id.Split('|')[2]))
                    ),
                    ImageData image => new ImageOutgoingSegment(
                        new(
                            new(image.File),
                            image.Summary,
                            image.Type switch
                    ...
                    _ => throw new NotSupportedException(
                        $"Unsupported outgoing segment type: {seg.GetType().Name}"
                    ),
                }
            ),
        ];

    private static ForwardOutgoingSegment ConvertFromCustomNodes(List<CustomNodeData> nodes) =>
        new(
            new ForwardOutgoingSegmentData([
                .. nodes.Select(node => new OutgoingForwardedMessage(
                    node.Sender,
                    node.Name,
                    ConvertFromMessageChain(node.Content)
                )),
            ])
        );
            ...
            if (result.EndOfMessage)
            {
                ...
                if (
                    await JsonSerializer.DeserializeAsync(
                        ms,
                        _milkyJsonSerializerContext.Event,
                        token
                    ) is
                    { } data
                )
    ...
    protected override async Task ExecuteAsync(CancellationToken token)
    {
        ...
        client = new MilkyClient(
            new HttpClient
            {
                BaseAddress = baseUri,
                DefaultRequestHeaders = { { "Authorization", $"Bearer {options.AccessToken}" } },
            }
        );
...
internal static partial class MilkyClientServiceLoggerExtension
{
    ...
    public static partial void LogConnectingWebSocket(
        this ILogger<MilkyClientService> logger,
        Uri uri
    );

    [LoggerMessage(Level = LogLevel.Information, Message = "Connected to websocket at {Uri}")]
    public static partial void LogConnectedWebSocket(
        this ILogger<MilkyClientService> logger,
        Uri uri
    );

    [LoggerMessage(Level = LogLevel.Warning, Message = "Websocket throws an exception")]
    public static partial void LogWebSocketException(
        this ILogger<MilkyClientService> logger,
    ...
    public static partial void LogUnknownSegmentType(
        this ILogger<MilkyClientService> logger,
        string segmentType
    );

    [LoggerMessage(Level = LogLevel.Warning, Message = "Unknown message scene: {MessageScene}")]
    public static partial void LogUnknownMessageScene(
        this ILogger<MilkyClientService> logger,
        string messageScene
    );
    ...
    [LoggerMessage(
        Level = LogLevel.Warning,
        Message = "Sending message failed due to internal server error, remaining tries: {TriesRemain}"
    )]
    public static partial void LogSendingMessageFailedDueToInternalServerError(
        this ILogger<MilkyClientService> logger,
        int triesRemain
    );
}
