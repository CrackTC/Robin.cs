...
internal class OneBotReactionEvent : OneBotNoticeEvent
{
    ...
    [JsonPropertyName("count")] public required uint Count { get; set; }

    public override BotEvent ToBotEvent(OneBotMessageConverter converter) =>
        SubType switch
        {
            "add" => new ReactionAddEvent(Time, MessageId.ToString(), GroupId, OperatorId, Code, Count),
            _ => new ReactionRemoveEvent(Time, MessageId.ToString(), GroupId, OperatorId, Code, Count)
        };
}
